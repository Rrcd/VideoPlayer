...
import com.example.videos.databinding.DetailsViewBinding
import com.example.videos.databinding.RenameFieldBinding
import com.example.videos.databinding.VideoMoreFeaturesBinding
import com.example.videos.databinding.VideoViewBinding
import java.io.File

class VAdp(private val context: Context, private var videoList: ArrayList<Video>, private var isFolder: Boolean = false)
    : RecyclerView.Adapter<VAdp.MyHolder>() {
    ...
    override fun onBindViewHolder(holder: MyHolder, @SuppressLint("RecyclerView") position: Int) {
        ...
        holder.root.setOnClickListener {
            when{
                videoList[position].id == PA.nowPlayingId -> {
...
//                MainActivity.dataChanged = true
                MainActivity.videoList = getAllVideos(context)
                FA.currentFolderVideos.removeAt(position)
                notifyDataSetChanged()
            }
            else -> {
...
//                notifyItemChanged(position)
            ...
            isFolder -> {
                FA.currentFolderVideos[position].title = newName
                FA.currentFolderVideos[position].path = newFile.path
                FA.currentFolderVideos[position].artUri = Uri.fromFile(newFile)
                notifyItemChanged(position)
//                MainActivity.dataChanged = true
                MainActivity.videoList = getAllVideos(context)
            }
