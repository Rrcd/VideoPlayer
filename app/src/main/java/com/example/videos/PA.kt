...
import com.example.videos.databinding.ActivityPlayerBinding
import com.example.videos.databinding.BoosterBinding
import com.example.videos.databinding.MoreFeaturesBinding
import com.example.videos.databinding.SpeedDialogBinding
import java.io.File
...
class PA : AppCompatActivity(), AudioManager.OnAudioFocusChangeListener, GestureDetector.OnGestureListener {
    ...
    private fun initializeLayout(){
        when(intent.getStringExtra("class")){
            ...
            "FolderActivity" -> {
                playerList = ArrayList()
                playerList.addAll(FA.currentFolderVideos)
                createPlayer()
    ...
    private fun initializeBinding(){
        ...
        binding.lockButton.setOnClickListener {
            if(!isLocked){
                ...
                binding.lockButton.setImageResource(R.drawable.close_lock_image)
            }
            else{
                //for showing
                isLocked = false
                ...
                binding.lockButton.setImageResource(R.drawable.lock_open_image)
            }
        }
    ...
    private fun playInFullscreen(enable: Boolean){
        if(enable){
            ...
            fullScreenBtn.setImageResource(R.drawable.fullscreen_exit_image)
        }else{
            binding.playerView.resizeMode = AspectRatioFrameLayout.RESIZE_MODE_FIT
            player.videoScalingMode = C.VIDEO_SCALING_MODE_SCALE_TO_FIT
            fullScreenBtn.setImageResource(R.drawable.fullscreen_image)
        }
    }
    ...
    override fun onPictureInPictureModeChanged(isInPictureInPictureMode: Boolean, newConfig: Configuration?) {
        if(pipStatus != 0){
            finish()
            val intent = Intent(this, PA::class.java)
            when(pipStatus){
